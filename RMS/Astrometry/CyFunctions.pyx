...
@cython.cdivision(False)
cpdef (double, double) equatorialCoordPrecession_2006_2(double start_epoch, double final_epoch, double ra, double dec):
    """ Corrects Right Ascension and Declination from one epoch to another, taking only precession into 
        account.
    ...
    # Time interval in Julian centuries
    t = (final_epoch - start_epoch) / 36525.0

    # Compute angles in radians
    gamma = (-0.052928 + 10.556378 * t + 0.4932044 * t * t - 0.00031238 * t * t * t - 0.000002788 * t * t * t * t + 0.0000000260 * t * t * t * t * t) * arcsec_to_rad
    phi = (84381.412819 - 46.811016 * t + 0.0511268 * t * t + 0.00053289 * t * t * t - 0.000000440 * t * t * t * t - 0.0000000176 * t * t * t * t * t) * arcsec_to_rad
    psi = (-0.041775 + 5038.481484 * t + 1.5584175 * t * t - 0.00018522 * t * t * t - 0.000026452 * t * t * t * t - 0.0000000148 * t * t * t * t * t) * arcsec_to_rad
    eps = (84381.406 - 46.836769 * t - 0.0001831 * t * t + 0.00200340 * t * t * t - 0.000000576 * t * t * t * t - 0.0000000434 * t * t * t * t * t) * arcsec_to_rad

    # Compute the precession matrix elements (we only need some of them)
    p11 = cos(psi) * cos(phi) * cos(gamma) - sin(psi) * cos(eps) * sin(gamma)
    p12 = -sin(psi) * cos(phi) * cos(gamma) - cos(psi) * cos(eps) * sin(gamma)
    p13 = -sin(eps) * cos(phi) * sin(gamma)
    p21 = cos(psi) * sin(gamma) + sin(psi) * cos(eps) * cos(gamma)
    p22 = -sin(psi) * sin(gamma) + cos(psi) * cos(eps) * cos(gamma)
    p23 = sin(eps) * cos(gamma)
    ...
    # Apply the precession matrix to initial coordinates
    ra_new = atan2(p21 * cos_ra + p22 * sin_ra, p11 * cos_ra + p12 * sin_ra)
    dec_new = asin(p13 * cos_dec + p23 * sin_ra * sin_dec + p21 * cos_ra * sin_dec)
